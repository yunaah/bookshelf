...
class _ShelvesPageState extends State<ShelvesPage> {
  ...
  // Fungsi Munculkan Dialog Tambah Rak
            ...
            ElevatedButton(
              onPressed: () async {
                if (shelfController.text.isNotEmpty) {
                  ...
                  if (success) {
                    setState(() {}); // Refresh UI Halaman Rak
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Rak berhasil dibuat!"), backgroundColor: Colors.green));
                  } else {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Gagal membuat rak"), backgroundColor: Colors.red));
                  }
                }
              },
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              bool success = await Data.deleteShelf(shelfName); // Panggil API Delete
              
              if (success) {
                setState(() {}); // Refresh UI
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Rak berhasil dihapus"), backgroundColor: Colors.red));
              } else {
                 ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Gagal menghapus rak"), backgroundColor: Colors.red));
              }
            }, 
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: ListView(
        ...
        children: [
          // HEADER DENGAN TOMBOL TAMBAH RAK (+)
          Row(
          ...
          // Loop untuk membuat Kartu Rak
          ...shelves.map((shelfName) {
            int count = Data.homeBookList.where((b) => b['shelf'] == shelfName).length;
            
            // CEK APAKAH INI RAK UNCATEGORIZED?
              ...
              child: Theme(
                ...
                child: ExpansionTile(
                  leading: Container(
                    width: 40, height: 40,
                    decoration: BoxDecoration(
                      // Bedakan warna icon kalau Uncategorized biar spesial
                  ...
                  // --- LOGIKA TOMBOL HAPUS ---
                  // Kalau Uncategorized, JANGAN tampilkan tombol hapus (null)
                  ...
                  // Isi Rak
                  children: Data.homeBookList.where((b) => b['shelf'] == shelfName).map((book) => Container(
